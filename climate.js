...
function renderMonthChart(climate) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    col.innerHTML = `
      <div class="month-label">${displayLabel}${yearLabel ? ` <span class="month-sublabel">${yearLabel}</span>` : ""}</div>
      <div class="metric-row">
        <span class="metric-icon">🌧️</span>
        <div class="metric-track"><div class="metric-fill rain" style="width:${stat.rainProb}%"></div></div>
        <span class="metric-pct">${stat.rainProb}%</span>
      </div>
      <div class="metric-row">
        <span class="metric-icon">❄️</span>
        <div class="metric-track"><div class="metric-fill snow" style="width:${stat.snowProb}%"></div></div>
        <span class="metric-pct">${stat.snowProb}%</span>
      </div>
    `;
